...
class CurveBall(Optimizer):
	...
	def __init__(self, params, auto_lambda=True, lambd=10.0,
			lambda_factor=0.999, lambda_low=0.5, lambda_high=1.5, lambda_interval=5,eps=1e-8):
		"""
		...
		"""
		defaults = dict(betaz=1e-3, rhoz=0.9, betas=1e-3, rhos=0.9, betay=1e-3, rhoy=0.9, auto_lambda=auto_lambda,
	...
	def step(self, model_fn, loss_fn):
		...
		for p in parameters:
			if p not in state:
				state[p] = {'z': t.zeros_like(p), 's': t.zeros_like(p), 'y': t.zeros_like(p)}
		
		zs = [state[p]['z'] for p in parameters]
		ss = [state[p]['s'] for p in parameters]
		ys = [state[p]['y'] for p in parameters]
		...
		"""
			precomputations for y before evalutation
		"""
		(Jy,) = fmad(predictions, parameters, ys)  # equivalent but slower	
		# compute loss Hessian (projected by Jy) using 2nd-order gradients
		(Hl_Jy,) = grad(Jl, predictions_d, grad_outputs=Jy, retain_graph=True)
		# compute J * (Hl_Jy) using RMAD (back-propagation).
		# note this is still missing the lambda * y + loss jacobian or ostrowski's term .***
		delta_ys = grad(predictions, parameters, Hl_Jy, retain_graph=True)
		# this is for the computation of hyper parameters 
		# we use current gradient as an approximation for the next gradient
		delta_y1 = grad(predictions, parameters, Hl_Jy+Jl_d, retain_graph=True)

		for (y, dy) in zip(ys, delta_y1):
			dy.data.add_(lambd, y)
		...
		if rho_z < 0 or beta_z < 0 or group['auto_lambda']:
			...
			(Hl_Jdeltas,) = grad(Jl, predictions_d, grad_outputs=Jdeltas, retain_graph=True)
			
			(Jdeltay,) = fmad(predictions, parameters, delta_y1)
			(Hl_Jdeltay,) = grad(Jl, predictions_d, grad_outputs=Jdeltay)
		...
		"""
			computations for step3
			forward and then calculation for ostrowski or any-other method
		"""
		predictions3 = model_fn()
		predictions3_d = predictions3.detach().requires_grad_(True)
		loss3 = loss_fn(predictions3_d)
		
		"""
			evaluation for delta_ys
		"""
		# now we need to calculate Jl2
		(Jl3,) = grad(loss3, predictions3_d, create_graph=True)
		Jl3_d = Jl3.detach()

		# now calculate Jl3Jphi3
		J_y = grad(predictions3, parameters, Jl3_d)

		# now calculate the term for 7th order and update them
		for (dz, ds, dy) in zip(J_z, J_s, J_y):
			...
			term3 = 4*(dy/(dz + t.sign(dz)*eps))
			dy.mul_(term1**2 + term2 + term3)

		# addition with delta_ss
		for (dy,j) in zip(delta_ys, J_y):
			dy.data.add_(j.detach())

		# add lambda * s term to the result, obtaining the final steps delta_ss
		for (y, dy) in zip(ys, delta_ys):
			dy.data.add_(lambd, y)
		
		"""
			finding hyper parameters for y
		"""
		beta_y, rho_y = group['betay'], group['rhoy']

		if beta_y < 0 or rho_y < 0 or group['auto_lambda']:  # required by auto-lambda
			# use previously computed Jdeltas and Hl_Jdeltas with approximation
			# solve 2x2 linear system: [rho, -beta]^T = [a11, a12; a12, a22]^-1 [b1, b2]^T.
			# accumulate components of dot-product from all parameters, by first aggregating them into a vector.
			
			y_vec = t.cat([y.flatten() for y in ys])
			dy_vec = t.cat([dy.flatten() for dy in delta_ys])

			a11 = lambd * (dy_vec * dy_vec).sum() + (Jdeltay * Hl_Jdeltay).sum()
			a12 = lambd * (dy_vec * y_vec).sum() + (Jy * Hl_Jdeltay).sum()
			a22 = lambd * (y_vec * y_vec).sum() + (Jy * Hl_Jy).sum()

			# for calculating b
			for (dy, dj, y) in zip (J_y, delta_ys, y):
				b1+=t.sum(dy*dj).item()
				b2+=t.sum(dy*y).item()
			# b1 = (Jl2_d * Jdeltass).sum()
			# b2 = (Jl2_d * Js2).sum()
			...
			beta_y = auto_params[0].item()
			rho_y = -auto_params[1].item()

		"""
			Parameter update
		"""
		for (p, y, dy) in zip(parameters, ys, delta_ys):
			y.data.mul_(rho_y).add_(-beta_y, dy)  # update state
			p.data.add_(y)
